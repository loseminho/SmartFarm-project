...
router.get("/:curpage", function(req, res){
  ...
  var querystring = 'select count(*) as cnt from sensor4;'
  ...
  client.query(querystring, function(error2, data){
    ...
    fs.readFile('views/sensor/sensor4.ejs', 'utf-8', function (error, data) {

    if (error) {
    console.log("ejs오류" + error);
    return
    }
    console.log("페이징 넘버 :" + no)

    var queryString = "SELECT id, sensor, DATA, date_format(TIME,'%Y-%m-%d %H:%i:%s')  as time from sensor4";
